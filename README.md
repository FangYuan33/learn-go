...
而在 Go 语言中，每个目录下的所有 `.go` 文件共享同一个 **包**，在包内可以定义多个类型、接口、函数和变量，如下为在 `user` 包下定义 User 和 Address 相关的内容，它们都被声明在一个 `user.go` 文件中：
...
相比来说，Java 代码组织的基本单元是类，并且更侧重对象定义，而 Go 代码组织的基本单元是包，并更侧重功能模块的聚合。

#### 可见性控制

在 Java 中是通过 `public/protected/private` 修饰符控制成员可见性的，而在 Go 语言中，通过 **首字母大小写** 控制“导出”（大写字母开头为 `public`），**包的导出成员对其他包可见**。以 user 包下 User 类型的定义为例，在 main 包下测试可见性如下：
...
Go 语言中有指针的概念，我们在这里说明一下：因为 Go 语言是 **“值传递”** 语言，方法/函数的形参（或接收器）中接收的实际上都是 **实参的副本**（如果不标记指针的话），那么在方法/函数中的操作并不会对原对象有影响，如果想对原对象进行操作，便需要通过指针获取到原对象才行（因为值传递会对原对象和形参对象都划分空间，所以针对较大的对象都推荐使用指针以节省内存空间）。在如下示例中，如果我们将 `double` 方法的形参修改为值传递，这样是不能将变量 a 扩大为两倍的，因为它操作的是 a 变量的副本：
